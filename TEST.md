...
Further reading on virtualenv
   ...
   2. Choose an empty folder (or create a new one). The folder I chose is ```C:\.TEMP\NLP```. Once you've seleted a folder, find your PowerShell window and type
```ps1
cd C:\FOLDER\OF\YOUR\CHOOSING
```  
In my case, it was
```ps1
cd C:\.TEMP\NLP
```  
   3. Before you install **virtualenv**, make sure you upgrade **pip** first by running
```ps1
...
```  
   5. Once installed, configure your virtual environment by running
```ps1
virtualenv.exe venv
```  
This will create a subfolder called **venv** and it's where your virtual Python environment will live.
   6. Enter your virtual environment by running
```ps1
.\venv\Scripts\activate.ps1
```  
```PS C:\``` should now be prepended with **(venv)** so that it looks like ```(venv) PS C:\```. <br>You are now operating within your Python virtual environment.
...
## NLTK_DATA Environment Variable  
   1. Strike WinKey ![](https://raw.githubusercontent.com/nstevens1040/NLP/main/images/winkey.svg), and type **powershell**. The top of your **Start Menu** should look like this.<br>![](https://raw.githubusercontent.com/nstevens1040/NLP/main/images/sm_ps.png)<br>Right click **Windows PowerShell** and then click **Run As Administrator**.
   2. In PowerShell, change directories to the folder you chose earlier. Type
```ps1
cd C:\FOLDER\OF\YOUR\CHOOSING
```  
In my case, it was
...
cd C:\.TEMP\NLP
```  
Strike **Enter**.<br>Then create subfolder called **NLTK_DATA**. Type
```ps1
mkdir NLTK_DATA
```  
Strike **Enter**.<br> This is the folder that you will set your NLTK_DATA environment variable to.
...
```  
```PS C:\``` should now be prepended with **(venv)** so that it looks like ```(venv) PS C:\```. <br>You are now operating within your Python virtual environment.
   4. Once you've entered your virtual environment, install **[scipy](https://scipy.org/)**.
```ps1
pip install scipy
```  
   5. Once scipy is installed, then install **[nltk](https://nltk.org)**
```ps1
pip install nltk
```  
   6. Once nltk is installed, then you may choose to:
      - Install only **popular** nltk data.
```ps1
python -m nltk.downloader popular
```  
      - Or you may choose to install **all** nltk data.
```ps1
